...
#
# However, when we do this, we also import `sloc_report/__init__.py'. If this
...
def get_project_files():
    """Retrieve a list of project files, ignoring hidden files and virtualenv
    directory.
...
def _test():
    ...
    # It also runs doctest, but only on the modules in UNIT_TESTS_DIRECTORY.
    return pytest.main(PYTEST_FLAGS + [UNIT_TESTS_DIRECTORY])


def _test_integration():
    """Run the integration tests (requires librato credentials to be exported
    ...
    import pytest
    # This runs the unit tests.
    # It also runs doctest, but only on the modules in
    # INTEGRATION_TESTS_DIRECTORY.
    return pytest.main(PYTEST_FLAGS + [INTEGRATION_TESTS_DIRECTORY])
...
setup_dict = dict(
    ...
    entry_points={
        'console_scripts': [
            'slocr = sloc_report.cli:cli_report'
        ],
    }
)
